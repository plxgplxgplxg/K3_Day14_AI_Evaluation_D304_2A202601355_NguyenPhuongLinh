...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        if relevant_hits == 0:
            return 0.0
        return min(1.0, max(0.0, precision_sum / relevant_hits))
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the answer against the rubric. Return JSON mapping each criterion "
            "to a 0-1 score.\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}"
        ...
        try:
            parsed = __import__("json").loads(reasoning)
        except Exception:
            return {"scores": default_scores, "reasoning": reasoning}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for record in scores_batch:
            ...
            positional_scores.append(average)

        overall_average = sum(averages) / len(averages) if averages else 0.0
        first_half = positional_scores[::2]
        second_half = positional_scores[1::2]
        positional_bias = (
            bool(first_half and second_half)
            and (sum(first_half) / len(first_half)) - (sum(second_half) / len(second_half)) > 0.1
        )
        return {
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        avg_faithfulness = (
            sum(result.faithfulness for result in results) / total if total else 0.0
        )
        avg_relevance = (
            sum(result.relevance for result in results) / total if total else 0.0
        )
        avg_completeness = (
            sum(result.completeness for result in results) / total if total else 0.0
        )
        recall_scores = [
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if values.count(scores[lowest_metric]) > 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[min(index - 1, len(suggestions) - 1)] if suggestions else "Investigate and add a targeted fix."
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0):
            suggestions.append(
                "Tighten retrieval grounding and add an unsupported-claim check before returning the final answer."
            )
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append(
                "Refine the answer prompt to restate the user intent and require every sentence to answer the question directly."
            )
        if categories.get("incomplete", 0):
            suggestions.append(
                "Add few-shot examples that preserve dates, amounts, conditions, and exceptions so answers stay complete."
            )

        if any(f.context_recall is not None and f.context_recall < 0.8 for f in failures):
            suggestions.append(
                "Improve retrieval coverage with better query expansion or larger top-k so the needed evidence is present."
            )
        if any(
            f.context_precision is not None and f.context_precision < 0.8
            for f in failures
        ):
            suggestions.append(
                "Rerank retrieved chunks by query overlap or a stronger reranker to move relevant evidence ahead of noise."
            )

        defaults = [
            "Add regression cases for the observed failures and block releases on repeated low-faithfulness or low-completeness patterns.",
            "Review chunking boundaries so multi-condition policies stay in the same retrieved evidence window.",
            "Log retrieval traces and answer drafts to diagnose whether failures come from missing context or weak synthesis.",
        ]
